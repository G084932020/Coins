...
//System.out.println("args[i].node="+args[i].node);
    ...
    */
    ...
    public boolean isStore(LirNode node) {
    	if(node.opCode == Op.SET && node.kid(0).opCode == Op.MEM && node.kid(1).opCode == Op.REG ) {
    		return true;
    	} else {
    		return false;
    	}
    ...
    public LirNode getIdx(LirNode exp) {
    	
    		return exp.kid(0).kid(1);
...
*/
    ...
    */
	...
	for(BiLink p=v.instrList().first();!p.atEnd();p=p.next()){
	    ...
	    System.out.println(getIdx(node));
	    }
	    }
	}
