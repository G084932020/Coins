    ...
    */
    ...
    boolean localCM(LirNode node, LirNode Addr, ArrayList<LirNode> vars, BasicBlk blk, BiLink q) {
    	...
    	for(BiLink p=q.next();!p.atEnd();p=p.next() ) {
    		...
    		//System.out.println(sameAddr(n,Addr));
		...
		for(int i=bVecInOrderOfRPost.length - 1;i > 0; i--) {
			...
			for(BiLink p=blk.instrList().last();!p.atEnd();p=p.prev()){
				...
				if(p!=blk.instrList().last()){
					//System.out.println(localCM(node,addr,vars,blk,p));
...
*/
    ...
    */
    ...
    /*BiLink pc=f.flowGraph().basicBlkList.first();
    BasicBlk Testbb=(BasicBlk)pc.elem();
    for(BiLink p=Testbb.instrList().first();!p.atEnd();p=p.next()){
    ...
    for(BiLink pp=f.flowGraph().basicBlkList.first();!pp.atEnd();pp=pp.next()){
        ...
        //System.out.println("");
	for(BiLink p=v.instrList().first().next();!p.atEnd();p=p.next()){
