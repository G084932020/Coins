...
public class MDGSA implements LocalTransformer {
	...
	*/
     ...
     **/
    public MDGSA(SsaEnvironment e, SsaSymTab tab, int m){
      env=e;
...
//System.out.println("args[i].node="+args[i].node);
    ...
    */
    ...
    public boolean isStore(LirNode node) {
    	return(node.opCode == Op.SET && node.kid(0).opCode == Op.MEM && node.kid(1).opCode == Op.REG );
    ...
    int checkIndexNum(LirNode exp1, LirNode exp2, int index){
		if(exp2.opCode==Op.SET) return checkIndexNum(exp1.kid(0).kid(0),exp2.kid(0).kid(0),index);
...
*/
    ...
    */
    ...
    //for (BiLink p = f.flowGraph().basicBlkList.first(); !p.atEnd(); p = p.next()) {
    	//BasicBlk Testbb = (BasicBlk)p.elem();
	
    	//System.out.println(Testbb.id);
    	//System.out.println(Testbb.instrList());
	...
	for(BiLink p=v.instrList().first();!p.atEnd();p=p.next()){
	    ...
	    System.out.println(getIndexNum(node));
	    }
